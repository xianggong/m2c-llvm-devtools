...
scpbin: 
	@-scp KERNEL_NAME.bin $(REMOTE_SERVER):$(REMOTE_PATH)/src/unitTest/KERNEL_PATH

run: scpbin
	@-ssh $(REMOTE_SERVER) "cd $(REMOTE_PATH)/src/unitTest/KERNEL_PATH; make && make run"

runisa: scpbin
	@-ssh $(REMOTE_SERVER) "cd $(REMOTE_PATH)/src/unitTest/KERNEL_PATH; make && make runm2sisa && scp debug.isa xgong@$(MYIP):$(CURDIR)"
